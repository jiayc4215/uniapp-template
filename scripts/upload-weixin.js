...
// 获取私钥路径
export function getPrivateKeyPath(appid) {
  // 查找私钥文件
  const keyPatterns = [`build/key/private.${appid}.key`, "build/ci_keys/private.key"]
...
export function uploadWeixin() {
  ...
  const envData = loadEnvFile("production")
  const appid = envData.VITE_WX_APPID
  if (!appid) {
  ...
  const miniCiConfig = {
    ...
    weixin: {
      appid,
      projectPath: path.relative(ROOT_DIR, projectPath),
      privateKeyPath: path.relative(ROOT_DIR, privateKeyPath),
      robot: params.robot,
      ignores: ["node_modules/**/*"],
      setting: {
        es6: false, // 是否启用 ES6 转 ES5 暂时会报错 see:https://developers.weixin.qq.com/community/develop/doc/000e8e37870090d4ba04acf9566000?highLine=_lruCache
        es7: false, // 是否启用 ES7 转 ES5 暂时会报错
        minify: true, // 是否压缩
        autoPrefixWXSS: true, // 是否自动前缀 WXSS
  ...
  const minicircPath = path.resolve(ROOT_DIR, ".minicirc")
  fs.writeFileSync(minicircPath, JSON.stringify(miniCiConfig, null, 2))

  try {
    execSync("minici --platform weixin", {
      cwd: ROOT_DIR,
      stdio: "inherit"
    })
    ...
    if (fs.existsSync(minicircPath)) {
      fs.unlinkSync(minicircPath)
    }
  }
}
uploadWeixin()
